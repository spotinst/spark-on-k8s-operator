...
func (c *Controller) submitSparkApplication(app *v1beta2.SparkApplication) *v1beta2.SparkApplication {
	...
	if c.metrics != nil {
		c.metrics.sparkSubmitCurrentCount.Inc()
	}
	submitStartTime := time.Now()
	submitted, err := runSparkSubmit(newSubmission(submissionCmdArgs, app))
	latency := time.Now().Sub(submitStartTime)
	if c.metrics != nil {
		c.metrics.sparkSubmitCurrentCount.Dec()
	}
	if err != nil {
		app.Status = v1beta2.SparkApplicationStatus{
			AppState: v1beta2.ApplicationState{
	...
	if c.metrics != nil {
		c.metrics.sparkSubmitLatency.Observe(float64(latency / time.Second))
	}
